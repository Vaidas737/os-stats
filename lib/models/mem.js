...
var Mem = {
  ...
  usage: function(cb) {
    ...
    // Windows support
      this.getUsageWin32(cb);    
    } else if (process.platform == "linux") {
    // Linux support
      this.getUsageLinux(cb); 
    } else {
    // Not supported :(
  ...
  /**
   *  Asynchronously calculate total memory usage on win32 platform.
   *  The callback is passed two arguments (err, result),
   *  where result is the total memory usage in percentage.
   *
   *  options: callback
   */
  getUsageWin32: function(cb) {
    // TODO: throw errors
    if (process.platform != "win32" || !isFunction(cb)) { return; }
  ...
  /**
   *  Asynchronously calculate total memory usage on linux platform.
   *  The callback is passed two arguments (err, result),
   *  where result is the total memory usage in percentage.
   *
   *  options: callback
   */
  getUsageLinux: function(cb) {
    // TODO: throw errors
    if (process.platform != "linux" || !isFunction(cb)) { return; }
